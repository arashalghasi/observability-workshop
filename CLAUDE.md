...
- Its **"Status of this checkout" table is the one section that rots silently.** Re-verify it against the tree rather than trusting it (`git status --short` plus a `Select-String` for `// LOG\.` in `PaymentResource.java`, `LongHistogram|WithSpan` in `PaymentService.java`). Current reality: the non-Java steps are applied and committed (`compose.yml` agent + Pyroscope + metric interval, `otelcol.yaml` redaction + tail sampling, `build.gradle.kts` otel deps, `application.yaml` MDC pattern) while **every `src/main/java` edit — 2.2, 2.3, 2.4-java, 3.5 meters, 4.3 annotations — is not applied**.
