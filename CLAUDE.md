...
- `WINDOWS-GUIDE.md` — long-form Windows setup writeup, local to this checkout and untracked upstream. Overlaps [On Windows](#on-windows); update both together when the Windows story changes. Formatting convention: **bold is the goal of the step** (the command to run, file to edit, UI to click — the 🛠️/📝/✅/🖥️ lead-in line above each block), normal weight is context (👀 observations, ⚠️ traps, rationale). Keep it when adding steps; don't bold inside fenced blocks — markdown can't, so the lead-in carries the emphasis. Each chapter opens with a `> **Scope** / **Why it matters** / **Done when** / **Concepts**` blockquote aimed at junior devs — keep that shape for new chapters.
- Its **"Status of this checkout" table is the one section that rots silently.** Re-verify it against the tree rather than trusting it (`git status --short` plus a `Select-String` for `// LOG\.` in `PaymentResource.java`, `LongHistogram|WithSpan` in `PaymentService.java`). As of the `init status` commit the checkout is a **clean slate**: no workshop step is applied, easypay is fully uninstrumented, and the only pre-provided pieces are the agent jars baked in by `easypay-service/src/main/docker/Dockerfile`. If a step gets applied later, add it to that table *and* fix the two chapter Scope boxes (Chapter 2 and Chapter 3) that state what is pre-applied.
- `OBSERVABILITY-101.md` — concepts companion to the Windows guide, written for a junior Java backend dev (the problem observability solves, four signals, MDC, cardinality, propagation, sampling, agent bytecode instrumentation, a "which signal do I reach for" table, glossary). Also local to this checkout. Every claim in it is tied to code in this repo, so re-check it when application behavior changes: thresholds (10000 / 20000 / 40000), the `@Retry` count, `POS-02`/`POS-03` seed rows, and the fact that refusals return HTTP 201 while only the `POS-02` NPE returns 500.
